...
def nnmain2(timeslice=0.01, tmax=0.05):
    #step 0: set up Neural network
    #step 1: Start pendulum with random force
    #step 2: at n milliseconds take state
    #step 3: pass state to genetic algorithm
    ...
    pendulum = InvertedPendulum()
    NN = NEvoNetwork(inputs=6, outputs=1, hiddenlayers=1,  hiddenneurons=5, inputweights=6, activation=TanhActivation)
    ga = Population(NN=NN, size=100)

    force = 300#np.random.randint(-100, 100)
    ...
    for n in range(0, 1000):
        for induhvidual in ga.individuals:
            ...
            airborne = True
            while airborne:
                force = NN.get_outputs([state[-1].x, state[-1].xdot, state[-1].x2dot, state[-1].theta, state[-1].thetadot, state[-1].theta2dot])[0] * 1000
                state, time = pendulum.get_State(u=force, initialstate=state[-1], tmax=tmax, timeslice=timeslice)

                #theta_array += state
                #print('Force = {0}, Theta = {1}'.format(force, state[-1].theta))
                ...
                #if induhvidual.get_fitness() == 5000: airborne = False
